...
export default function MenuCard({menuItems, isNested = false, lastIndex=0}) {
    ...
    if("categories" in menuItems){
        return (<>
            <div className="w-full max-w-200 mx-auto border-b-8 border-gray-100 pb-4">
                <p className="text-xl font-extrabold text-black py-4">{menuItems?.title}</p>
                <div>
                {menuItems?.categories?.map((category, index) => (
                    <MenuCard key={category?.title} menuItems={category} isNested={true} lastIndex={index==menuItems.categories.length-1} />
                ))}
                </div>
            </div>
        </>)
